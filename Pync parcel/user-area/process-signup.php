<?php
	...
	$FName =  mysqli_real_escape_string($conn, htmlspecialchars($_REQUEST['fname'])) ;
	$LName = mysqli_real_escape_string($conn, htmlspecialchars($_REQUEST['lname'])) ;
    ...
    if(  !isset($_REQUEST['signup']) ){
	    ...
	    header("location: ../sign-up.php?FName=$FName&LName=$LName&Number=$Number&Emailaddress=$Emailaddress"); 
        die();
    }
	...
	else if ( !preg_match("/$pwd/", $VerifyPwd ) ) {
    ...
    $sql = "INSERT INTO users (FName, LName, PhoneNumber, EmailAddress, Password_Hash, AccountNumber, Create_At) 
    VALUES ('$FName', '$LName', ' $Number', '$Emailaddress', '$hash_password', '$Account_Number', '$created_at')"; 
   
    if(mysqli_query($conn,  $sql)){ 
	
	    $last_id = mysqli_insert_id($conn);
	    $sql = "INSERT INTO  authorized_users (Id) value($last_id)";
		if( mysqli_query($conn, $sql) ){ 
		
		    	 //$_SESSION['message'] = "Registration successful. You can now log in"; 
	             header("location: ../../sendmail.php?Fname=$FName&Account_Number=$Account_Number&signUp_email=$Emailaddress");  
                 die(); 
		}else{ 
